...
def main() -> int:
    ...
    try:
        ...
        from PySide6.QtWidgets import (QMainWindow, QLabel, QVBoxLayout, QHBoxLayout, 
                                     QWidget, QPushButton, QTabWidget, QTextEdit, 
                                     QListWidget, QGroupBox, QGridLayout, QProgressBar,
        ...
        def new_recording():
            ...
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 📝 新規記録を開始します")
        ...
        def open_recording():
            ...
            if file_path:
                ...
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 📂 記録ファイルを開きました: {file_path}")
        ...
        def save_current():
            ...
            if file_path:
                ...
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 💾 記録を保存しました: {file_path}")
        ...
        def show_settings():
            ...
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ⚙ 設定画面を開きます")
        ...
        def show_log_viewer():
            ...
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 📝 ログビューアーを表示します")
        ...
        def show_about():
            ...
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ℹ バージョン情報を表示します")
...
<p><i>© 2025 EZRPA Development Team</i></p>
        ...
        welcome_label.setStyleSheet("font-size: 16px; font-weight: bold; color: #0078D4;")
        ...
        def start_recording():
            ...
            # 新しい記録名を生成
            current_recording_name = f"recording_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    ...
                    if sys.platform == "win32":
                        ...
                        try:
                            ...
                            if result.returncode == 0:
                                success = True
                                logger.info("🗕 Shell.Application経由ですべてのウィンドウを最小化しました")
                                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🗕 記録開始：すべてのウィンドウを最小化しました")
                        ...
                        if not success:
                            try:
                                ...
                                success = True
                                logger.info("🗕 Windows+D経由ですべてのウィンドウを最小化しました")
                                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🗕 Windows+D経由ですべてのウィンドウを最小化しました")
                        ...
                        if not success:
                            try:
                                ...
                                success = True
                                logger.info("🗕 pynput経由ですべてのウィンドウを最小化しました")
                                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🗕 pynput経由ですべてのウィンドウを最小化しました")
                        ...
                        if not success:
                            ...
                            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - WARNING - ⚠️ ウィンドウ最小化に失敗しました")
                            
                    else:
                        # 非Windows環境では何もしない
                        logger.info("非Windows環境のため、ウィンドウ最小化をスキップしました")
                        
                except Exception as e:
                    logger.warning(f"ウィンドウ最小化処理でエラー: {e}")
                    log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - WARNING - ⚠️ ウィンドウ最小化エラー: {e}")
            ...
            def on_action_recorded(action: RPAAction):
                ...
                action_count_label.setText(f"記録アクション数: {recording_state.action_count}")
                ...
                action_name = action_type_names.get(action.action_type, action.action_type)
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - DEBUG - 🎯 {action_name}: {action.data}")
            ...
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 📹 実際のRPA記録を開始しました: {current_recording_name}")
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🖱️ マウスとキーボードの操作をリアルタイムで記録中...")
            ...
            def update_recording_time():
                if recording_state.is_recording and not recording_state.is_paused:
                    ...
                    recording_time_label.setText(f"記録時間: {int(hours):02d}:{int(minutes):02d}:{int(seconds):02d}")
            ...
            status_bar.showMessage("📹 実際のRPA記録中 - マウスとキーボードの操作をリアルタイムで記録しています...")
        ...
        def stop_recording():
            ...
            if success:
                ...
                # ログ出力
                logger.info(f"⏹ 実際のRPA記録を停止しました - {recording_state.action_count}アクション記録")
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ⏹ 実際のRPA記録を停止しました - {recording_state.action_count}アクション記録")
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 💾 記録ファイルを保存しました: recordings/{current_recording_name}.json")
                
                status_bar.showMessage(f"✅ RPA記録完了 - {current_recording_name} ({recording_state.action_count}アクション)")
            else:
                logger.error("記録停止に失敗しました")
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - ERROR - ❌ 記録停止に失敗しました")
            ...
            def restore_main_window():
                try:
                    ...
                    log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 📋 記録停止：EZRPAウィンドウを前面に復元しました")
                except Exception as e:
            ...
            if floating_window:
                ...
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🗑️ フローティングウィンドウを非表示にしました")
        ...
        def pause_resume_recording():
            ...
            if recording_state.is_paused:
                ...
                pause_recording_btn.setStyleSheet("QPushButton { background-color: #ffc107; color: black; padding: 10px; }")
                ...
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ▶ RPA記録を再開しました")
                ...
                pause_recording_btn.setStyleSheet("QPushButton { background-color: #28a745; color: white; padding: 10px; }")
                ...
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ⏸ RPA記録を一時停止しました")
                status_bar.showMessage("⏸ 一時停止中 - 再開ボタンでRPA記録を続行できます")
        
        start_recording_btn = QPushButton("🔴 記録開始")
        start_recording_btn.setStyleSheet("QPushButton { background-color: #dc3545; color: white; font-weight: bold; padding: 10px; }")
        start_recording_btn.setToolTip("マウス・キーボード操作の記録を開始します\n記録開始時にすべてのウィンドウが最小化されます")
        start_recording_btn.clicked.connect(start_recording)
        
        stop_recording_btn = QPushButton("⏹ 記録停止")
        stop_recording_btn.setStyleSheet("QPushButton { background-color: #6c757d; color: white; padding: 10px; }")
        stop_recording_btn.setToolTip("記録を停止して保存します\n停止時にEZRPAウィンドウが前面に復元されます")
        stop_recording_btn.setEnabled(False)
        ...
        def refresh_recordings_list():
            ...
            if available_recordings:
                ...
                recordings_list.addItem("🔍 記録がありません - 記録タブで新しい記録を作成してください")
        ...
        def play_recording():
            ...
            if not selected_items:
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.warning(main_window, "警告", "再生する記録を選択してください。")
                return
            
            selected_text = selected_items[0].text()
            if "記録がありません" in selected_text:
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.information(main_window, "情報", "記録タブで新しい記録を作成してください。")
                return
            ...
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ▶ 実際のRPA再生を開始: {recording_name}")
            
            # 進捗コールバック設定
            def on_playback_progress(current, total):
                if total > 0:
                    progress = int((current / total) * 100)
                    log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - DEBUG - 🎬 再生進捗: {current}/{total} ({progress}%)")
            ...
            def on_playback_complete():
                ...
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ✅ RPA再生が完了しました")
            ...
            if rpa_manager.play_recording(recording_name, speed_multiplier=1.0):
                ...
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🎬 マウスとキーボード操作を自動実行中...")
            else:
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.warning(
                    main_window, 
        ...
        def stop_playback():
            ...
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ⏹ RPA再生を停止しました")
        ...
        def edit_recording():
            ...
            if not selected_items:
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.warning(main_window, "警告", "編集する記録を選択してください。")
                return
            
            selected_recording = selected_items[0].text()
            logger.info(f"✏ 記録の編集を開始: {selected_recording}")
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ✏ 記録の編集を開始: {selected_recording}")
            
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.information(main_window, "編集", f"記録エディターを開きます:\n{selected_recording}\n\n(この機能は今後実装予定です)")
        ...
        def delete_recording():
            ...
            if not selected_items:
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.warning(main_window, "警告", "削除する記録を選択してください。")
                return
            ...
            if reply == QMessageBox.Yes:
                ...
                logger.info(f"🗑 記録を削除しました: {selected_recording}")
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🗑 記録を削除しました: {selected_recording}")
                status_bar.showMessage(f"🗑 削除完了: {selected_recording}")
        
        play_btn = QPushButton("▶ 再生")
        play_btn.setStyleSheet("QPushButton { background-color: #28a745; color: white; font-weight: bold; padding: 8px; }")
        play_btn.clicked.connect(play_recording)
        
        stop_playback_btn = QPushButton("⏹ 停止")
        stop_playback_btn.setStyleSheet("QPushButton { background-color: #dc3545; color: white; padding: 8px; }")
        stop_playback_btn.setEnabled(False)
        stop_playback_btn.clicked.connect(stop_playback)
        
        edit_btn = QPushButton("✏ 編集")
        edit_btn.setStyleSheet("QPushButton { background-color: #007bff; color: white; padding: 8px; }")
        edit_btn.clicked.connect(edit_recording)
        
        delete_btn = QPushButton("🗑 削除")
        delete_btn.setStyleSheet("QPushButton { background-color: #dc3545; color: white; padding: 8px; }")
        ...
        def add_schedule():
            ...
            # 簡単なスケジュール追加ダイアログ
            schedule_name, ok = QInputDialog.getText(main_window, "スケジュール追加", "スケジュール名を入力してください:")
            if ok and schedule_name:
                ...
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ➕ 新しいスケジュールを追加しました: {schedule_name}")
        ...
        def edit_schedule():
            ...
            if not selected_items:
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.warning(main_window, "警告", "編集するスケジュールを選択してください。")
                return
            
            selected_schedule = selected_items[0].text()
            logger.info(f"✏ スケジュールの編集を開始: {selected_schedule}")
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ✏ スケジュールの編集を開始: {selected_schedule}")
            
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.information(main_window, "編集", f"スケジュールエディターを開きます:\n{selected_schedule}\n\n(この機能は今後実装予定です)")
        ...
        def delete_schedule():
            ...
            if not selected_items:
                from PySide6.QtWidgets import QMessageBox
                QMessageBox.warning(main_window, "警告", "削除するスケジュールを選択してください。")
                return
            ...
            if reply == QMessageBox.Yes:
                ...
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🗑 スケジュールを削除しました: {selected_schedule}")
                status_bar.showMessage(f"🗑 削除完了: {selected_schedule}")
        
        add_schedule_btn = QPushButton("➕ スケジュール追加")
        add_schedule_btn.setStyleSheet("QPushButton { background-color: #28a745; color: white; padding: 8px; }")
        add_schedule_btn.clicked.connect(add_schedule)
        
        edit_schedule_btn = QPushButton("✏ 編集")
        edit_schedule_btn.setStyleSheet("QPushButton { background-color: #007bff; color: white; padding: 8px; }")
        edit_schedule_btn.clicked.connect(edit_schedule)
        
        delete_schedule_btn = QPushButton("🗑 削除")
        delete_schedule_btn.setStyleSheet("QPushButton { background-color: #dc3545; color: white; padding: 8px; }")
        ...
        log_text.setStyleSheet("QTextEdit { font-family: 'Consolas', monospace; font-size: 10px; }")
...
"""
        ...
        def clear_log():
            ...
            log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🗑 ログがクリアされました")
        ...
        def refresh_log():
            ...
            if log_file_path.exists():
                try:
                    ...
                    log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🔄 ログが更新されました")
                    status_bar.showMessage("🔄 ログ更新完了")
                except Exception as e:
                    logger.error(f"ログファイル読み込みエラー: {e}")
                    log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - ERROR - ログファイル読み込みエラー: {e}")
            else:
                logger.info("🔄 ログファイルが見つかりません")
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 🔄 ログファイルが見つかりません")
        ...
        def export_log():
            ...
            if file_path:
                try:
                    ...
                    log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - 💾 ログをエクスポートしました: {file_path}")
                    status_bar.showMessage(f"💾 エクスポート完了: {file_path}")
                except Exception as e:
                    logger.error(f"ログエクスポートエラー: {e}")
                    log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - ERROR - ログエクスポートエラー: {e}")
        ...
        status_bar.showMessage(f"準備完了 - {APP_NAME} v{APP_VERSION} | Clean Architecture | Windows専用")
        ...
        def check_rpa_status():
            ...
            if not rpa_status['recording_supported']:
                from datetime import datetime
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - WARNING - ⚠️ RPA機能が制限されています")
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - WARNING - 📦 pynput ライブラリをインストールしてください: pip install pynput")
                status_bar.showMessage("⚠️ RPA機能制限 - pynput ライブラリが必要です")
            else:
                from datetime import datetime
                log_text.append(f"{datetime.now().strftime('%H:%M:%S')} - INFO - ✅ RPA機能が利用可能です")
