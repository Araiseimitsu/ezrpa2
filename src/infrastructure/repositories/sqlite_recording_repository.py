...
class SqliteRecordingRepository(IRecordingRepository):
        ...
        try:
            with self._lock:
                ...
                if self._encryption_service.is_encryption_available():
                    encrypt_result = self._encryption_service.encrypt_data(
                        recording_json
                    )
                ...
                """,
                    (
                        ...
                        now,
                        (
                            recording.completed_at.isoformat()
    ...
    def _json_to_recording(
        self, json_data: str, actions: List[ActionTypes]
    ) -> Recording:
        """JSONから記録オブジェクトを復元"""
        ...
        try:
            with self._lock:
                ...
                if not row:
                    return Err(
                        ErrorInfo(
                            "RECORDING_NOT_FOUND", f"記録が見つかりません: {name}"
                        )
                    )
            ...
            return Err(
                ErrorInfo(
                    "RECORDING_GET_BY_NAME_ERROR", f"名前による記録取得エラー: {str(e)}"
                )
            )

    async def get_by_date_range(
        self, start_date: datetime, end_date: datetime
    ) -> Result[List[Recording], ErrorInfo]:
        """日付範囲でレコーディングを取得"""
        try:
            with self._lock:
                conn = self._get_connection()
            ...
            return Err(
                ErrorInfo(
                    "RECORDING_GET_BY_DATE_ERROR", f"日付範囲記録取得エラー: {str(e)}"
                )
            )
    ...
    async def update(self, recording: Recording) -> Result[bool, ErrorInfo]:
        ...
        try:
            ...
            if save_result.is_success():
                ...
                return Err(
                    ErrorInfo(
                        "RECORDING_UPDATE_ERROR", f"記録更新エラー: {save_result.error}"
                    )
                )
        except Exception as e:
    ...
    async def exists(self, recording_id: str) -> Result[bool, ErrorInfo]:
        ...
        try:
            ...
            return Err(
                ErrorInfo("RECORDING_EXISTS_ERROR", f"記録存在確認エラー: {str(e)}")
            )
    ...
    async def count(self) -> Result[int, ErrorInfo]:
        ...
        try:
            ...
            return Err(
                ErrorInfo("RECORDING_COUNT_ERROR", f"記録数取得エラー: {str(e)}")
            )
    ...
    async def count_by_status(self, status: RecordingStatus) -> Result[int, ErrorInfo]:
        ...
        try:
            with self._lock:
                conn = self._get_connection()
                cursor = conn.execute(
                    "SELECT COUNT(*) as count FROM recordings WHERE status = ?",
                    (status.value,),
                )
                count = cursor.fetchone()["count"]
                return Ok(count)
            ...
            return Err(
                ErrorInfo(
                    "RECORDING_COUNT_BY_STATUS_ERROR",
                    f"ステータス別記録数取得エラー: {str(e)}",
                )
            )

    # バックアップ・復元機能
    async def backup_to_file(
        self, file_path: Path, recording_ids: Optional[List[str]] = None
        ...
        try:
            ...
            if write_result.is_success():
                ...
                return Err(
                    ErrorInfo(
                        "RECORDING_BACKUP_ERROR",
                        f"バックアップファイル書き込みエラー: {write_result.error}",
                    )
                )

        except Exception as e:
            return Err(
                ErrorInfo("RECORDING_BACKUP_ERROR", f"記録バックアップエラー: {str(e)}")
            )

    async def restore_from_file(
        self, file_path: Path, overwrite: bool = False
        ...
        try:
            ...
            if read_result.is_failure():
                return Err(
                    ErrorInfo(
                        "RECORDING_RESTORE_READ_ERROR",
                        f"復元ファイル読み込みエラー: {read_result.error}",
                    )
                )
            ...
            if "recordings" not in backup_data:
                return Err(
                    ErrorInfo(
                        "RECORDING_RESTORE_FORMAT_ERROR", "無効な復元ファイル形式です"
                    )
                )
            ...
            return Err(
                ErrorInfo("RECORDING_RESTORE_ERROR", f"記録復元エラー: {str(e)}")
            )

    # Windows環境固有機能
    async def export_to_windows_task(
        self, recording_id: str, task_name: str
    ) -> Result[bool, ErrorInfo]:
        ...
        try:
            ...
            return Err(
                ErrorInfo(
                    "RECORDING_EXPORT_TASK_ERROR",
                    f"Windowsタスクエクスポートエラー: {str(e)}",
                )
            )
        ...
        try:
            with self._lock:
                ...
                storage_info = {
                    ...
                    "avg_actions_per_recording": (
                        round(
                            stats["total_actions"] / max(stats["recording_count"], 1), 2
                        )
            ...
            return Err(
                ErrorInfo(
                    "RECORDING_STORAGE_INFO_ERROR",
                    f"ストレージ情報取得エラー: {str(e)}",
                )
            )
    ...
    async def optimize_storage(self) -> Result[Dict[str, Any], ErrorInfo]:
        ...
        try:
            ...
            return Err(
                ErrorInfo(
                    "RECORDING_OPTIMIZE_ERROR", f"ストレージ最適化エラー: {str(e)}"
                )
            )
        ...
        try:
            ...
            with self._lock:
                ...
                for mismatch in count_mismatches:
                    warnings.append(
                        f"アクション数不整合: 記録ID={mismatch['recording_id']}, 期待={mismatch['action_count']}, 実際={mismatch['actual_count']}"
                    )
            ...
            return Err(
                ErrorInfo("RECORDING_VALIDATION_ERROR", f"整合性検証エラー: {str(e)}")
            )
