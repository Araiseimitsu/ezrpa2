...
class INPUT(ctypes.Structure):
    ...
    _fields_ = [
        ...
        ("union", ctypes.c_char * 24)  # KEYBDINPUT, MOUSEINPUT, HARDWAREINPUTの最大サイズ
    ]
...
class WindowsApiService:
    ...
    def send_key_input(self, key_code: int, shift: bool = False, 
                      ctrl: bool = False, alt: bool = False, 
                      win: bool = False) -> Result[None, str]:
        """キー入力を送信"""
        try:
            with self._lock:
                # 修飾キーの押下
    ...
    def send_mouse_input(self, x: int, y: int, button: int = 1, 
                        double_click: bool = False, 
                        wheel_delta: int = 0) -> Result[None, str]:
        """マウス入力を送信"""
        try:
            with self._lock:
                # DPIスケーリング調整
    ...
    def find_window(self, title: Optional[str] = None, 
                   ...
                   process_name: Optional[str] = None) -> Result[WindowInformation, str]:
        """ウィンドウを検索"""
        try:
    ...
    def move_window(self, handle: int, x: int, y: int, 
                   width: Optional[int] = None, 
                   height: Optional[int] = None) -> Result[None, str]:
        """ウィンドウを移動・リサイズ"""
        try:
            if not self._user32.IsWindow(handle):
                return Err("無効なウィンドウハンドルです")
            ...
            success = self._user32.MoveWindow(handle, scaled_x, scaled_y, 
    ...
    def _send_text_unicode(self, text: str) -> Result[None, str]:
        ...
        try:
            for char in text:
                ...
                hwnd = self._user32.GetForegroundWindow()
                if hwnd:
                    self._user32.PostMessageW(hwnd, WindowsMessages.WM_CHAR, char_code, 0)
