...
class PlaybackApplicationService:
    ...
    def __init__(self,
        ...
        self._play_recording_use_case = PlayRecordingUseCase(
            recording_repository, settings_repository, keyboard_adapter, mouse_adapter
        )
        self._pause_playback_use_case = PausePlaybackUseCase(self._play_recording_use_case)
        self._stop_playback_use_case = StopPlaybackUseCase(self._play_recording_use_case)
        self._get_playback_status_use_case = GetPlaybackStatusUseCase(self._play_recording_use_case)
        self._resume_playback_use_case = ResumePlaybackUseCase(self._play_recording_use_case)
        self._validate_recording_use_case = ValidateRecordingUseCase(recording_repository)
    ...
    async def start_playback(self, 
                           recording_id: str, 
                           config: PlaybackConfigDTO) -> Result[str, ErrorInfo]:
        """
        再生を開始する
        ...
        try:
            ...
            if len(self._active_sessions) >= self._max_concurrent_playbacks:
                return Err(ErrorInfo("MAX_CONCURRENT_PLAYBACKS", f"同時再生数上限に達しています: {self._max_concurrent_playbacks}"))
            
            # 記録の検証
            validation_result = await self._validate_recording_use_case.execute(recording_id)
            if validation_result.is_failure():
                return validation_result
            
            validation = validation_result.value
            if not validation['is_playable']:
                return Err(ErrorInfo("RECORDING_NOT_PLAYABLE", f"記録が再生できません: {', '.join(validation['errors'])}"))
    ...
    async def pause_playback(self, session_id: str) -> Result[bool, ErrorInfo]:
        ...
        try:
            ...
            return Err(ErrorInfo("PAUSE_PLAYBACK_ERROR", f"再生一時停止エラー: {str(e)}"))
    ...
    async def stop_playback(self, session_id: str) -> Result[PlaybackResultDTO, ErrorInfo]:
        """
        再生を停止する
    ...
    async def get_playback_status(self, session_id: Optional[str] = None) -> Result[PlaybackStatusDTO, ErrorInfo]:
        """
        再生ステータスを取得する
        ...
        try:
            ...
            # 全セッションの場合（最初のアクティブセッション）
                ...
                if not self._active_sessions:
                    return Err(ErrorInfo("NO_ACTIVE_SESSION", "アクティブなセッションがありません"))
                
                first_session_id = next(iter(self._active_sessions.keys()))
                return await self.get_playback_status(first_session_id)
            
        except Exception as e:
            return Err(ErrorInfo("GET_PLAYBACK_STATUS_ERROR", f"ステータス取得エラー: {str(e)}"))
    
    async def validate_recording_for_playback(self, recording_id: str) -> Result[PlaybackValidationDTO, ErrorInfo]:
        """
        記録の再生可能性を検証する
        ...
        try:
            ...
            validation_dto = PlaybackValidationDTO(
                ...
                estimated_duration_seconds=validation_data['estimated_duration_seconds']
            )
            
            return Ok(validation_dto)
            
        except Exception as e:
            return Err(ErrorInfo("VALIDATE_RECORDING_ERROR", f"記録検証エラー: {str(e)}"))
    
    async def add_to_queue(self, 
                          recording_id: str, 
                          config: PlaybackConfigDTO,
                          priority: int = 0,
                          scheduled_time: Optional[datetime] = None) -> Result[str, ErrorInfo]:
        """
        再生キューに追加する
        ...
        try:
            ...
            self._playback_queue.queue_items.sort(key=lambda x: (-x.priority, x.created_at))
    ...
    async def get_queue_status(self) -> Result[PlaybackQueueDTO, ErrorInfo]:
        ...
        try:
            ...
            return Err(ErrorInfo("GET_QUEUE_STATUS_ERROR", f"キュー状態取得エラー: {str(e)}"))
    ...
    async def process_queue(self) -> Result[List[str], ErrorInfo]:
        ...
        try:
            ...
            for queue_item in self._playback_queue.pending_items:
                ...
                # 実行時刻チェック
                if queue_item.scheduled_time and queue_item.scheduled_time > datetime.now(timezone.utc):
                    continue
                
                # 再生開始
                session_result = await self.start_playback(
                    queue_item.recording_id,
                    queue_item.config
                )
    ...
    async def get_playback_history(self, 
                                  recording_id: Optional[str] = None,
                                  limit: int = 100) -> Result[PlaybackHistoryDTO, ErrorInfo]:
        """
        再生履歴を取得する
        ...
        try:
            ...
            if recording_id:
                filtered_history = [h for h in filtered_history if h.recording_id == recording_id]
            ...
            return Err(ErrorInfo("GET_PLAYBACK_HISTORY_ERROR", f"履歴取得エラー: {str(e)}"))
    ...
    def _update_performance_metrics(self):
        ...
        if self._playback_history:
            ...
            self._performance_metrics['average_duration'] = total_duration / len(self._playback_history)
