...
class RecordingApplicationService:
    ...
    def __init__(self,
        ...
        self._get_all_recordings_use_case = GetAllRecordingsUseCase(recording_repository)
        ...
        self._get_recordings_by_status_use_case = GetRecordingsByStatusUseCase(recording_repository)
        
        # キャッシュ（簡易実装）
        self._cache = {}
        self._cache_timeout = 300  # 5分
    
    async def create_recording(self, create_dto: CreateRecordingDTO) -> Result[str, ErrorInfo]:
        """
        新しい記録を作成する
    ...
    async def start_recording(self, recording_id: str) -> Result[bool, ErrorInfo]:
        ...
        try:
            ...
            if recording.status == RecordingStatus.RECORDING:
                return Err(ErrorInfo("RECORDING_ALREADY_STARTED", "記録は既に開始されています"))
    ...
    async def stop_recording(self, recording_id: str) -> Result[RecordingDTO, ErrorInfo]:
        """
        記録を停止する
    ...
    async def add_action(self, recording_id: str, action: ActionTypes) -> Result[bool, ErrorInfo]:
        """
        記録にアクションを追加する
    ...
    async def get_recording(self, recording_id: str, include_actions: bool = True) -> Result[RecordingDTO, ErrorInfo]:
        """
        記録を取得する
    ...
    async def get_all_recordings(self, 
                                page: int = 1, 
                                page_size: int = 50,
                                status_filter: Optional[RecordingStatus] = None) -> Result[RecordingListDTO, ErrorInfo]:
        """
        全記録を取得する（ページング対応）
        ...
        try:
            # 記録の取得
            if status_filter:
                result = await self._get_recordings_by_status_use_case.execute(status_filter)
            else:
    ...
    async def search_recordings(self, search_dto: RecordingSearchDTO) -> Result[RecordingListDTO, ErrorInfo]:
        """
        記録を検索する
        ...
        try:
            ...
            result = await self._search_recordings_use_case.execute(
                search_dto.query, 
                search_dto.page_size * search_dto.page  # 簡易実装
            )
    ...
    async def update_recording(self, recording_id: str, update_dto: UpdateRecordingDTO) -> Result[RecordingDTO, ErrorInfo]:
        """
        記録を更新する
