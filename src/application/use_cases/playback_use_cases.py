...
class PlayRecordingUseCase:
    ...
    async def execute(self, recording_id: str, 
                     speed_multiplier: float = 1.0,
                     start_from_action: int = 0) -> Result[str, ErrorInfo]:
        """
        記録を再生する
    ...
    async def _load_playback_config(self, speed_multiplier: float) -> Dict[str, Any]:
        ...
        # 設定から値を取得
        delay_result = await self._settings_repository.get("playback.default_delay", 500)
        if delay_result.is_success():
            config['default_delay'] = delay_result.value
        
        stop_on_error_result = await self._settings_repository.get("playback.stop_on_error", True)
        if stop_on_error_result.is_success():
            config['stop_on_error'] = stop_on_error_result.value
        
        screenshots_result = await self._settings_repository.get("playback.take_screenshots", False)
...
class PausePlaybackUseCase:
    ...
    async def execute(self, session_id: str) -> Result[bool, ErrorInfo]:
        ...
        try:
            # セッションの確認
            if (not self._play_use_case._current_playback or 
                self._play_use_case._current_playback['session_id'] != session_id):
                return Err(ErrorInfo("INVALID_PLAYBACK_SESSION", "無効な再生セッションです"))
            ...
            self._play_use_case._current_playback['pause_time'] = datetime.now(timezone.utc)
    ...
    async def execute(self, session_id: str) -> Result[Dict[str, Any], ErrorInfo]:
        ...
        try:
            ...
            result = {
                ...
                'completion_rate': current['current_action_index'] / len(current['recording'].actions),
                'status': 'completed' if current['current_action_index'] >= len(current['recording'].actions) else 'stopped'
            }
...
class GetPlaybackStatusUseCase:
    ...
    async def execute(self, session_id: Optional[str] = None) -> Result[Dict[str, Any], ErrorInfo]:
        """
        再生ステータスを取得する
        ...
        try:
            ...
            if current:
                ...
                status_info.update({
                    ...
                    'total_actions': len(current['recording'].actions),
                    'progress_percentage': (current['current_action_index'] / len(current['recording'].actions)) * 100,
                    'elapsed_seconds': (now - current['start_time']).total_seconds(),
                    'speed_multiplier': current['config']['speed_multiplier']
                })
                
                # 一時停止中の場合は一時停止時間も追加
                if self._play_use_case._playback_status == PlaybackStatus.PAUSED and current['pause_time']:
                    status_info['pause_time'] = current['pause_time']
                    status_info['paused_duration_seconds'] = (now - current['pause_time']).total_seconds()
            
            return Ok(status_info)
            
        except Exception as e:
            return Err(ErrorInfo("GET_PLAYBACK_STATUS_ERROR", f"再生ステータス取得エラー: {str(e)}"))
    ...
    async def execute(self, recording_id: str) -> Result[Dict[str, Any], ErrorInfo]:
        ...
        try:
            ...
            if recording.status != RecordingStatus.COMPLETED:
                ...
                validation_result['errors'].append(f"記録が完了していません: {recording.status.value}")
            ...
            for i, action in enumerate(recording.actions):
                try:
                    ...
                    if not hasattr(action, 'action_type'):
                        validation_result['warnings'].append(f"アクション {i}: 不明なアクション形式")
                    
                    # 座標の範囲チェック（マウス操作の場合）
                    if hasattr(action, 'position'):
                        pos = action.position
                        if pos.x < 0 or pos.y < 0 or pos.x > 9999 or pos.y > 9999:
                            validation_result['warnings'].append(f"アクション {i}: 座標が範囲外の可能性があります ({pos.x}, {pos.y})")
                
                except Exception as e:
                    validation_result['warnings'].append(f"アクション {i}: 検証中にエラー: {str(e)}")
