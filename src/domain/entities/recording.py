...
from ..value_objects import (
    RecordingStatus, Duration, ValidationResult, 
    CommonDurations, FileInfo
)
...
class Recording:
    ...
    def add_action(self, action: ActionTypes) -> Result[None, str]:
        ...
        if len(self.actions) >= ValidationConstants.MAX_ACTIONS_PER_RECORDING:
            return Err(f"アクション数の上限({ValidationConstants.MAX_ACTIONS_PER_RECORDING})に達しています")
        ...
        if not validation_result.is_valid:
            return Err(f"アクションバリデーションエラー: {', '.join(validation_result.errors)}")
    ...
    def validate(self) -> ValidationResult:
        ...
        if not self.name or len(self.name.strip()) == 0:
            ...
            errors.append(f"記録名が長すぎます（{ValidationConstants.MAX_RECORDING_NAME_LENGTH}文字以内）")
        ...
        if len(self.actions) > ValidationConstants.MAX_ACTIONS_PER_RECORDING:
            errors.append(f"アクション数が上限を超えています（{ValidationConstants.MAX_ACTIONS_PER_RECORDING}個以内）")
        
        # 個別アクションのバリデーション
        for i, action in enumerate(self.actions):
            action_validation = action.validate()
            if not action_validation.is_valid:
                errors.extend([f"アクション{i+1}: {error}" for error in action_validation.errors])
            warnings.extend([f"アクション{i+1}: {warning}" for warning in action_validation.warnings])
        
        # 再生設定のバリデーション
        settings_validation = self.playback_settings.validate()
        if not settings_validation.is_valid:
            errors.extend([f"再生設定: {error}" for error in settings_validation.errors])
        warnings.extend([f"再生設定: {warning}" for warning in settings_validation.warnings])
    ...
    def clone(self, new_name: Optional[str] = None) -> 'Recording':
        ...
        cloned = Recording(
            ...
            playback_settings=PlaybackSettings.from_dict(self.playback_settings.to_dict())
        )
    ...
    def can_be_executed(self) -> bool:
        """実行可能かどうか"""
        return (self.status == RecordingStatus.COMPLETED and 
    ...
    def can_be_edited(self) -> bool:
        """編集可能かどうか"""
        return self.status in [RecordingStatus.CREATED, RecordingStatus.RECORDING, RecordingStatus.PAUSED]
